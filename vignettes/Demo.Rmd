...
First, it is handy to translate the phases into numbers, for which we should conveniently choose the 'actual' dating. I know this causes other problems in the end, since such phases are often employed to _avoid_ dates, but it is necessary in this case and may a well be changes back later. Ideally, we can produce a 'beginning' and 'end' date for each object, or let's say an earliest possible dating and a latest possible dating corresponding to beginning and stark of each phase mentioned above. 
...
In my opinion, things that can be dated with greater condidence should have a larger impact on the overall visualisation. That is why the following function productes a columns named 'weight' which contains a value that corresponds to the timespan covered by the two dating variables. The greater the timespan, the lower the weight value. 
...
```{r scaleweight}
df <- scaleweight(df, df$Technique)
kable(head(df))
```

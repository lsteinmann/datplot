...
The manually curated data that B. Weissová prepared for her dissertation was saved as an Excel-file[^1]. As this is very often the case, we will continue directly from this original file, which B. Weissová kindly permitted to be published along this package. The original file is available [in this repositories (redacted)](https://github.com/anonymousgit/datplot) "inst/extdata/"-subfolder. It can be loaded into R without further complications using the `read_excel()`-function from the package `readxl`. Alternatively, the file is suited for storage as a *.tsv-file, which is also provided in the same folder for better long-term support. 
...
The next step is sorting out values from the `Dating` variable that have to be manually entered, such as the "Roman Imperial Period". We achieve this by excluding all values that contain a number, preparing a table in which we can manually enter the desired dating span and saving it as a .csv-file. After adding the values manually, we reload the file. The manually edited concordance can be found in the "inst/extdata/"-subfolder [of this repository (redacted)](https://github.com/anonymousgit/datplot) in the file `periods_edit.csv` and corresponds to the chronological assessment of periods used in the original publication [@weissova2019, 42].
